...
int get_coord_ion(int index, int tp, int *r)
{
	...
	for(i=0;i<3;i++)
		r1[i]=rand()%(2*max_lat[i]+1)-max_lat[i];
...
int create_single_chain()
{
	...
	for(i=0;i<Nchain;i++){
		...
		l=rand()%2;
		tmp=pow(-1.0, l);
	//	cout<<i+1<<"\t"<<j<<"\t"<<l<<"\t"<<tmp<<endl;
		for(l=0;l<3;l++){
			r1[l]=r[l];
		}
		r1[j]+=tmp;
	//	isoccupied=0;
		isoccupied=is_occupied(i+1,r1);
		}

		for(j=0;j<3;j++) {r[j]=r1[j]; tmp=abs(r[j]);
		if(tmp>max_lat[j]) max_lat[j]=tmp;
		}
	}
...
{
	ofstream out;
	int i,j,k,l, trans[3], molec_index, atom_index, tp=2, ch=1, ngroups;
	int maxr[3]={0,0,0};
	...
	for(i=0;i<Nchain;i++)
	{
		...
		for(j=0;j<3;j++){
			out<<chain1[i].r[j]<<" ";
			if(abs(chain1[i].r[j])>maxr[j]) maxr[j]=abs(chain1[i].r[j]);
		}
		out<<endl;
	}
	}

	cout<<"Polyanion created\n";
	molec_index=Npolym+1; atom_index=Npolym*Nchain+1;
	//create the cations
	ch=-1;  //cation charge
	for(i=0;i<Ncation;i++)
	{
	l=get_coord_ion(i, 0, trans); //0 indicates cation
	cation[i].assign(atom_index++, molec_index++, tp, ch, trans); 
	out<<cation[i].index<<" "<<cation[i].molec<<" "<<cation[i].type<<" "<<cation[i].charge*QFAC<<" ";
	for(j=0;j<3;j++){
			if(abs(cation[i].r[j])>maxr[j]) maxr[j]=abs(cation[i].r[j]);
		out<<cation[i].r[j]<<" ";
		}
	out<<endl;	
	...
	cout<<"MAX "<<maxr[0]<<"\t"<<maxr[1]<<"\t"<<maxr[2]<<endl;

	//Create the anions
/*	ch=-1;
	for(i=0;i<Nanion;i++)
	{
	l=get_coord_ion(i, 1, trans);
	anion[i].assign(atom_index++, molec_index++, tp, ch, trans); 	
	out<<anion[i].index<<" "<<anion[i].molec<<" "<<anion[i].type<<" "<<anion[i].charge*QFAC<<" ";
	for(j=0;j<3;j++)
		out<<anion[i].r[j]<<" ";
	out<<endl;		
	...
	for(i=0;i<ngroups;i++)
	{
		l=0;
			if(i==0)
			out<<k++<<" "<<1<<" "<<i*11+l+1<<" "<<i*11+l+2<<endl;
			else
			out<<k++<<" "<<1<<" "<<i*11-5<<" "<<i*11+l+2<<endl;
			

		for(l=1;l<5;l++)
			out<<k++<<" "<<1<<" "<<i*11+l+1<<" "<<i*11+l+2<<endl;

			out<<k++<<" "<<1<<" "<<i*11+2<<" "<<i*11+7<<endl;

			out<<k++<<" "<<1<<" "<<i*11+3<<" "<<i*11+8<<endl;
			out<<k++<<" "<<1<<" "<<i*11+8<<" "<<i*11+9<<endl;

			out<<k++<<" "<<1<<" "<<i*11+5<<" "<<i*11+10<<endl;
			out<<k++<<" "<<1<<" "<<i*11+10<<" "<<i*11+11<<endl;		
			out<<k++<<" "<<1<<" "<<i*11+10<<" "<<i*11+12<<endl;										
	}
	
			out<<k++<<" "<<1<<" "<<i*11-5<<" "<<Nchain<<endl;
...
{	
	...
	for(i=0;i<3;i++)
		out<<tmp[i]<<" ";
	out<<"\n#Pair definition\n";
	out<<"special_bonds\tlj 0 1 1  coul 1 1 1\n";
	in>>str>>tmp[0];
	if(res)
...
/*	if(res==0 || res==1){
		//lammps mas restriction of 32 groups
		if(2*Npolym <=32){
			for(k=1;k<=Npolym;k++)
				out<<"group polymer"<<k<<" molecule "<<k<<endl;
		}	
	}
*/
	out<<"fix 1 all nve\n";
...
/*	if(res==0 || res==1)
	{
		if(2*Npolym <=32){
		for(k=1;k<=Npolym;k++)
		out<<"fix "<<k+2<<" polymer"<<k<<" recenter INIT INIT INIT units box\n";
		}
	}
*/
	out<<"\n#Dump\n#------------------------------------------------\n";
...
void read_config(char *str)
{
	...
	Ncation=Npolym*Nchain; Nanion=0;
...
int main(int argc, char **argv)
{
	...
	j=atoi(argv[3]); //For equilibriation step set 0 otherwise 1
		...
		cout<<"Enter config file name and random number\n";
		return 0;
	}
